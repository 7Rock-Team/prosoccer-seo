...
### 2026-08-04 - Live store carries many concurrent packs of the same model+tier+cut+surface

**Finding:** For footwear, "model + tier + cut + surface" does NOT resolve to one live product. The live sitemap shows several concurrent packs of the identical shoe. Every one of the six Batch 12 Nike Academy/Club SKUs has at least one concurrent live senior pack sibling on a different pack name: HQ2278 phantom 6 high academy FG/MG (Shadow FA26) vs Breakout SU26 + Attack SP26; HQ2277 high academy turf (Shadow FA26) vs Scary Good FA25; HJ4564 low academy FG/MG (Shadow FA26) vs Shadow FA25; IO1494 superfly 11 academy turf (Shadow FA26) vs Break 'Em; IM0358 superfly 11 club FG/MG (Shadow FA26) vs Break 'Em; IB1600 tiempo maestro academy FG/MG (Shadow FA26) vs Attack SP26.
**Evidence:** firecrawl_map over prosoccer.com (sitemap include), searches "phantom 6 academy" and "tiempo maestro academy", 2026-08-04. DFS (kw_data_google_ads_search_volume + dataforseo_labs_google_keyword_overview, 2026-08-04): pack-qualified terms are near-universally zero volume (only "nike phantom 6 shadow pack" scrapes 50/mo, launch-inflated); model+tier demand lives on unqualified terms (adidas f50 elite 6600, adidas predator elite fg 1000, nike phantom 6 pro 880), and is brand-shaped (adidas holds volume down to model+tier+surface; Nike model+tier+surface strings return zero even unqualified).
**Strategic implication:** (1) The pre-dispatch cannibalization check reads the registry only, which holds optimized pages; the concurrent incumbents here are unregistered, so the registry-only check cannot see them. Footwear pack collisions must be checked against the live sitemap, not just the registry. This is the same exact-match/registry-only blind spot noted in SEO_BATCH_PROCESS.md, now with a live example. (2) Merchandising gaps flagged for Jorge (do not invent collections): `nike-phantom-6` (only `nike-phantom` exists, generation-spanning), `adidas-predator-elite`, `adidas-f50-elite` (tier-level), and a Nike Vapor / Superfly split (only `nike-mercurial` exists). Interim: land generation/tier terms on the next-broader existing collection. (3) Pack-succession rule under discussion with Mike (narrower form: qualify only when a concurrent live pack sibling exists); NOT codified pending his approval of the wording.
...
### 2026-05-08 - Subagent MCP runtime gap: ROOT CAUSE IDENTIFIED AND FIXED

**Finding:** The Mexico-run gap was caused by frontmatter `tools:` field syntax. Each specialist's `tools:` line listed MCPs as human-readable shorthand: `Google Drive MCP, Tavily MCP, DataForSEO MCP, Firecrawl MCP, Playwright MCP, GSC MCP`. Claude Code parses the `tools:` field as a comma-separated list, which split each shorthand into multiple invalid tokens (e.g., `Google Drive MCP` → `Google`, `Drive`, `MCP`). None of those tokens match real Claude Code tool names, so subagents inherited zero MCP access. Symptom matched exactly: Read/Write/Edit/Glob/Grep/Bash worked; every MCP failed silently.

**Evidence:** Confirmed by inspecting the agents-list block surfaced at session start: `(Tools: Read, Write, Edit, Glob, Grep, Bash, Google, Drive, MCP, Tavily, MCP, ...)` for each specialist. Real MCP server names exist as `mcp__claude_ai_Google_Drive`, `mcp__claude_ai_Tavily`, `mcp__plugin_playwright_playwright`, `mcp__dfs-mcp`, `mcp__firecrawl-mcp`, `mcp__gsc-server` per `.claude/settings.local.json` allow list and the deferred-tool inventory.

**Fix applied 2026-05-08:** Updated frontmatter `tools:` line in all five agent definitions (KIRA, VERITAS, SCRIBE, RECON, ORIN) to use proper `mcp__server-name` prefixes that grant access to all tools from each MCP server. Each prefix is a valid Claude Code subagent tool token.

**Verification caveat:** Claude Code caches agent definitions at session start. The repaired `tools:` field will surface MCPs to subagents starting with the next fresh Claude Code session, not retroactively in the session that applied the fix. First Wave 2 multi-agent run is the operational verification opportunity. If subagents in that run still report missing MCPs, the hypothesis is wrong and the architecture-decision path (b), streamlined ORIN-with-playbooks, becomes the recommended fallback.

**Strategic implication:** If the fix holds, the consolidate-don't-fragment architecture stays viable at Wave 2 scale and beyond. Specialist-vs-ORIN token ratio should rebalance toward 1:1 or specialist-heavy, freeing ORIN context for routing and consolidation work. If the fix doesn't hold in fresh-session testing, ORIN escalates to Mike for path (b) decision and `work-log/follow-ups.md` entry remains open.
