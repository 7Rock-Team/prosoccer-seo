...
- **Gate 12: Editorial sanity scan (image precision + parallel construction + supporting keyword, added 2026-06-02).** Judgment-call sanity scan, not strict pattern matching. Flag (a) muddy imagery -- evocative sentences failing the "what's the actual image?" test (unclear physical action, vague temporal sequence, disconnected cause-and-effect); (b) inconsistent parallel construction across any list of 3+ examples (mixed possessive / descriptor / quote / preposition usage); (c) multiple supporting keywords used in body copy where the rule is ONE supporting keyword at 3 to 5 mentions. Route flagged items back to SCRIBE for revision. See `context/workforce-conventions.md` 'Image precision (cross-cutting)', 'Parallel construction (cross-cutting)', 'Supporting keyword selection (cross-cutting)'.
