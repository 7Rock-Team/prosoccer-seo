...
1. **Read all SKUs in the pack first** (the submitted set, grouped by silo / pack / series).
2. **Draft a per-SKU differentiation spec.** For each SKU, pre-assign a unique angle of emphasis, a unique opening-hook approach, a unique heritage / positioning angle, a unique use-case scenario, and a unique primary metaphor or scene framing. No two siblings share a lane.
3. **Dispatch one SCRIBE agent per SKU with that SKU's lane named in the dispatch prompt.** SCRIBE produces from the differentiation spec, not from the exemplar's prose; the exemplar still supplies structure (H2 count and order, bullet placement, FAQ count, field-length tiers).

The pass is judgment-based, not script-enforceable. It refines "mirror the exemplar" into "mirror the exemplar's structure; produce from your assigned lane." The defense-in-depth re-check (Section 9 batch dispatch, step 5) backstops it with a pairwise prose comparison across sibling briefs (~40% similarity flag). Cross-reference: `context/workforce-conventions.md` 'Pack/series coordination discipline', 'Cross-brief prose uniqueness discipline', and 'Keyword cannibalization discipline'; SCRIBE-side production note in `.claude/agents/on-page-seo/agent.md` Section 9. Production source: Day 3 batch commit 088ae19 review.
