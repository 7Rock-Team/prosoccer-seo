...
1. **Read Registry 1 (white-label keyword sheet) via the parent-level Drive connector.** URL https://docs.google.com/spreadsheets/d/1H-4Ax8C6IbfqCx2SToVidD4p9GR_rn16PePuvGMSA6Q/edit. For each SKU in the batch, find its row (PDPs tab for products, Collections tab for collections) and capture status, any existing primary-keyword assignment, and date. Filter a large sheet to silo-relevant rows plus exact URL matches. ORIN reads at the PARENT level and injects the relevant rows into each SCRIBE dispatch context; SCRIBE never calls Drive (the OAuth token does not propagate to sub-agents, per the Category B inheritance gap in `context/workforce-conventions.md` 'Architectural notes').
...
- **Mechanism B (backstop): forbidden-phrasings list.** ORIN also extracts the exemplar's CLAIMED PHRASINGS (its specific H2 titles, its definitional sentences for concepts that recur across siblings such as the FG / AG / tier / plate definitions, its primary metaphor, its opening hook, its closing line) and adds them to each sibling's lane spec as a forbidden list to write around. The backstop catches propagation that survives despite skeleton-only handoff, for instance when SCRIBE's own familiarity with the silo gravitates to a similar phrasing independently.
...
1. **Produce both files per batch.** Alongside the per-SKU brief files (`<slug>_brief.md`, implementer content only), ORIN produces one `_audit-trail.md` at the session-folder root carrying batch metadata and the per-SKU audit notes (complexity classification, keyword research with volumes, brand-IP classification, sibling-title uniqueness, internal-link validation evidence, the differentiation lane ORIN assigned at pre-dispatch, defense-in-depth gate notes, URL-handle flags). The differentiation lane ORIN already drafts in the pre-dispatch pass feeds straight into each SKU's audit-trail entry.
2. **Surface the Current live Title to SCRIBE.** The brief's Quick Reference Current live Title field comes from the Phase 0 Firecrawl scrape. ORIN confirms the live title is captured and reaches the brief so Mike searches Shopify admin by title, not by SKU.
