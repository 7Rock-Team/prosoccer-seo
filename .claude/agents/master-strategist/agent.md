...
4. **Dispatch one SCRIBE agent per SKU with that SKU's lane spec in the prompt.** SCRIBE produces from the lane spec, not from the exemplar's prose; the exemplar still supplies structure (H2 count and order, bullet placement, FAQ count, field-length tiers). The lane spec already carries the keyword and silo-positioning context SCRIBE needs, so SCRIBE does not read either registry directly.
5. **Defense-in-depth at gate.** Pairwise prose comparison across the batch (Section 9 batch dispatch, step 5); flag any pair exceeding ~40% prose similarity.
