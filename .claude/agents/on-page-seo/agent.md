...
0.5. **Eligibility verification (added 2026-05-27, exception types expanded 2026-05-28).** Before steps 1 through 11, verify the candidate page (PDP or collection) is eligible for optimization per the page-type playbook eligibility section. Detection: Firecrawl scrape the candidate URL, extract stock status (PDP) or product card presence (collection), confirm 200 OK with real page content (not soft-404, not auto-redirect). Apply `context/page-type-playbooks/product-page-playbook.md` 'Eligibility verification (mandatory pre-Phase-1)' for PDPs and `context/page-type-playbooks/collection-page-playbook.md` 'Eligibility verification (mandatory pre-Phase-1)' for collections. If eligibility fails, surface as BLOCKER and hold at gate; do not proceed to Step 1 or any subsequent brief production work. Strategic exceptions for PDPs: (a) closing-window optimization for end-of-life, closeout, or discontinued-generation inventory with retained collector or completist value (restock not expected); (b) pre-tournament demand spike optimization for current-cycle sold-out inventory with imminent tournament or seasonal demand event (typically 60 days or less) and expected restock during or after the event window. Strategic exception for collections: seasonal empty collections optimized ahead of product drops. All exception overrides require explicit Mike approval with the exception type named and strategic reasoning logged in the session briefing under `.claude/agents/on-page-seo/briefings/`. Don't silently optimize an ineligible page. Documented exception examples: Liverpool 2024-25 Nike Away Jersey v2 (commit b7159dc, closing-window), adidas Predator Accuracy.1 FG Crazyrush Pack v2 (commit d52e56f, closing-window), Mexico 2026 kit set Stadium SS Home/Away/Third (2026-05-28 codification, pre-tournament demand spike, 2026 World Cup co-host kickoff June 11). Decision-logic summary lives in `context/page-type-playbooks/product-page-playbook.md` 'Decision logic for strategic exceptions'.
