...
0.5. **Eligibility verification (added 2026-05-27).** Before steps 1 through 11, verify the candidate page (PDP or collection) is eligible for optimization per the page-type playbook eligibility section. Detection: Firecrawl scrape the candidate URL, extract stock status (PDP) or product card presence (collection), confirm 200 OK with real page content (not soft-404, not auto-redirect). Apply `context/page-type-playbooks/product-page-playbook.md` 'Eligibility verification (mandatory pre-Phase-1)' for PDPs and `context/page-type-playbooks/collection-page-playbook.md` 'Eligibility verification (mandatory pre-Phase-1)' for collections. If eligibility fails, surface as BLOCKER and hold at gate; do not proceed to Step 1 or any subsequent brief production work. Strategic exception (closing-window for PDPs, seasonal empty for collections) requires explicit Mike override documented in the session briefing under `.claude/agents/on-page-seo/briefings/`. Don't silently optimize an ineligible page. Documented exception examples (predating codification): Liverpool 2024-25 Nike Away v2 (commit b7159dc), adidas Predator Accuracy.1 FG Crazyrush Pack v2 (commit d52e56f).
