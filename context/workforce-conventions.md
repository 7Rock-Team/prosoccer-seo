...
**5. Sub-floor lock, not route-back.** When a live sibling forces qualification, the pack-qualified sub-floor primary is MANDATORY and correct. Do not walk the fallback hierarchy down to a floor-clearing term, and do not route back to KIRA: a less-specific term collides with the incumbent or the sibling. This is the case `SEO_BATCH_PROCESS.md` section 3 names ("when no floor-clearing term is hierarchy-valid, the page takes the exact qualified term and is flagged sub-floor"), applied to packs. It is a deliberate exception to the general "route back rather than lock a sub-floor primary" posture in the Volume-Weighted Primary Keyword Selection Discipline above and in `.claude/agents/master-strategist/agent.md` Section 9.
