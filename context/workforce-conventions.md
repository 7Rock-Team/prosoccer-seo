...
7. Validate every proposed internal link via the firecrawl skill (status code 200, page-type signals confirmed, no soft-404) per the matching playbook's link strategy (1 to 2 max). **A status code alone is never a validation: read the target's H1 and product count, and confirm its AUDIENCE matches the linking page.** A 200 on a wrong-audience collection is worse than a 404, because a 404 gets noticed and a wrong-audience 200 ships silently. Two incidents: the `/collections/youth-soccer-shoes` false 404, and `/collections/indoor-soccer-shoes` (2026-08-13), which returns 200 with 32 products under the H1 "Kids' Indoor Soccer Shoes" and would have taken three adult Batch 14 indoor SKUs to youth product. The adult target is `/collections/indoor`. Record the H1 and product count in the input file's validated-links block. Full rule and both worked cases: `context/page-type-playbooks/product-page-playbook.md` 'Live validation requirement'.
...
### Generation tokens (carried, added 2026-08-13)

A GENERATION token is the manufacturer's name for the current iteration of a model line, carried in the live product title: `Hyperfast` (adidas F50), `NU3`, `LV8`, `Flex`. It is not a pack and not a colorway, and it is not styled all-caps (see the brand technology rule below). Generation tokens are CARRIED in the meta title, ahead of the pack qualifier, because they are how searchers name the product: GSC 2026-08-13 showed all seven registered adidas F50 incumbents earning zero impressions on the terms they hold while every impression they did earn came from `hyperfast` queries, one at position 4.4. Whether the token also belongs in the PRIMARY keyword is open and undecided, logged as `strategy/sprint-backlog.md` B-KW-01; Batch 14 shipped primaries WITHOUT it by Mike's decision, so nothing is pre-committed. Full ordering, the pack-yields-first tiebreak, the meta-description corollary, and a worked example: `context/page-type-playbooks/product-page-playbook.md` 'Meta title priority ordering, and the generation token'.

### Brand technology names (title case, added 2026-08-13)
...
Expect this to recur wherever ProSoccer's import pipeline normalizes manufacturer copy. **Known instance:** all-caps technology names, which flattened `Nanostrike+` and `Nanostrike` into one string and hid a tier distinction (see the Nanostrike rule above). **Predicted next instance, logged 2026-08-19 so the pattern is visible when it turns up: TRUNCATED PRODUCT TITLES.** The `<title>` truncation already confirmed as a theme-level bug is the same shape of loss: where a title is cut, any qualifier at the end of it (a pack name, a season code, a cut token, a width) disappears from the page while remaining real on the product. Two pages that differ only in a trailing qualifier then present identically, which is exactly condition 1 of the test above. **If you find two SKUs whose live titles are identical but whose configurations differ, suspect truncation before you suspect a duplicate**, and take the qualifier from the manufacturer source or the handle rather than concluding the pages are the same product. Recording the prediction now means the next occurrence gets recognized as an instance rather than investigated from scratch.
...
**Authoring caveat (added 2026-08-03, Batch 11): a forbidden entry must never be a substring of an approved phrasing.** The verbatim and title-frame tiers are enforced by SUBSTRING match (`check_forbidden_phrasings` / `check_cross_brief`), so a barred string that is contained inside a phrasing the claims bar APPROVES will fire on the approved copy and produce a false FAIL. Worked example (JZ3102, Batch 11): the Bayern claims bar (Real-Madrid-strict) barred the bare superlative `germany's most storied club`, written into the JZ3070/JZ3102 input `gate-meta.forbidden_phrasings.verbatim`. But the Mike-approved qualitative phrasing is `one of Germany's most storied clubs`, which CONTAINS `germany's most storied club` as a substring. SCRIBE used the approved plural form correctly, yet the gate failed JZ3102 on `forbidden-verbatim`. Fix: the barred string was removed from both input files; the bare-superlative risk stays guarded by the claims-posture prose and the Layer 3 heritage read. Rule for ORIN when writing a forbidden list: before adding a verbatim or title-frame entry, confirm it is NOT a substring of any phrasing the claims bar explicitly approves. If the only precise target is a bare form that an approved phrasing embeds, do not encode it as a substring bar; leave it to the claims posture, the Layer 3 heritage check, and the `heritage-superlative` regex where that applies.

**Corollary: a brand token must NEVER be a forbidden-verbatim entry (added 2026-08-03, Batch 12; SECOND instance of the substring trap).** The KJ3432 (adidas F50) input barred `Adidas` to catch a capitalized brand name. But `check_forbidden_phrasings` matches case-INSENSITIVELY (`s.lower() in text.lower()`), and `Adidas` is a substring of the legitimate lowercase `adidas` (present in every field of an adidas product) and of audit compounds like `non-Adidas`, so the bar fired 12 times on correct copy and hard-failed the gate. A capitalized brand token is already enforced elsewhere: `voice_check.py` `\bAdidas\b` = FAIL, plus ORIN's orchestrator-layer brand-styling re-check. Rule for ORIN writing any `gate-meta.forbidden_phrasings`: a verbatim or title-frame list must never contain a brand name (adidas, Nike, Mizuno, and so on). The general substring rule above subsumes it, but brand tokens are the sharpest case, because the approved form differs from the barred form only by case and the check is case-insensitive, so the false FAIL is guaranteed.
...
**Hierarchy bound and sub-floor lock (added 2026-08-04, doctrine settled by Batch 9).** The walk is subordinate to the keyword hierarchy: every broader candidate must still resolve to ONE product to stay PDP-eligible. The moment broadening would take a term that describes multiple products (a collection or a sibling term), STOP. When no floor-clearing candidate stays hierarchy-valid, the page LOCKS the exact qualified term flagged sub-floor; it does not broaden into a collection or sibling term for volume, and it does not route back to a broader term. Volume never overrides hierarchy (`SEO_BATCH_PROCESS.md` section 3, "when no floor-clearing term is hierarchy-valid, the page takes the exact qualified term and is flagged sub-floor"). In footwear this lock is the norm, not the exception, because one step of broadening usually crosses into a multi-pack or multi-surface (collection) term; see 'Pack succession and PDP keyword ownership (v2)'. An unmeasurable term is absent, not winnable, so a locked sub-floor term is accepted because the hierarchy forces it, not because it will earn traffic on its own; the model-level demand is captured by the collection.
...
- **Primary:** unchanged, per Mechanism A (volume-weighted + GSC override, head-term weight). Exception (pack succession, 2026-08-04): when a concurrent live pack sibling exists for the same model + tier + cut + surface, the pack-qualified long-tail is the PRIMARY, not a secondary, and the newer pack takes it (see 'Pack succession and PDP keyword ownership (v2)').
...
## The live title governs; the handle is never a source of product attributes (added 2026-08-25, Mike)
...
**PHASE 0 IMPLEMENTATION (added 2026-08-27, Mike approved).** `scripts/phase0_product_facts.py` does the extraction; `scripts/test_phase0_product_facts.py` covers it with 16 tests built from real captured payloads. The module is pure so it can be tested offline: fetch `/products/<handle>.json`, pass the decoded object to `extract(product, expected_sku=...)`.

It returns `colorway`, `is_apparel`, `colorway_absent_by_design`, `size_option_name`, `size_values`, `base_skus` and `verified_sku`, and `input_line(facts)` emits the exact line for the per-SKU input file. Three behaviours are deliberate and tested:

- **Apparel writes an explicit NOT APPLICABLE sentence, never a blank.** `- Colorway (Color option): NOT APPLICABLE. This is apparel and apparel PDPs carry no Color option. Absent by design, not a gap.` A future reader must not have to guess whether a blank on a jersey is a store fact or a scrape failure. Blank is ambiguous; a sentence is not.
- **Footwear with no `Color` option ESCALATES rather than defaulting.** That combination is unexpected and the module says so in the input line rather than emitting an empty field. Silence plus a plausible value is the failure mode item 11 exists to prevent.
- **`verify_sku()` raises `SkuMismatch`, it does not warn.** Variant SKUs are `<BASE-SKU>-<size>`, so the base SKU is recoverable and the SKU we were handed can be checked against the page actually fetched. **This is nearly free and it would have caught the Batch 16 18282 handle-versus-SKU contradiction automatically instead of by inspection.** A test asserts that handing UF1F16X's SKU to its colorway sibling FAILS, because a check that cannot fail the way the subject failed is not a check (codification checklist item 13).
...
- `adidas-jr-f50-hyperfast-mid-fg-mg-soccer-cleats-chaos-vs-control` (KJ0670) carries **no tier token at all**. Read from the handle it looks like a League Mid page and collides with KK1316. The live title is `adidas Junior F50 Hyperfast Club Mid Firm/Multi Ground Soccer Cleats - Chaos Vs Control Pack (FA26)`. It is a CLUB page, its real pack sibling is the Club Mid Immortal DNA page, and its correct incumbency determination depends entirely on knowing that.
- `adidas-kids-f50-hyperfast-club-fg-mg-soccer-cleats-chaos-vs-control` (KK1345) omits **Velcro**, which is the closure that distinguishes it from the laced Junior page in the same pack at the same tier and surface. The live title is `adidas Kids' F50 Hyperfast Club Velcro Firm/Multi Ground Soccer Cleats - Chaos Vs Control (FA26)`, sizes 8K to 13.5K.

Prior instances of the same class, already known and now collected here as one rule rather than four separate cautions: handles write `jr` where titles write `Junior`; `kids` where titles write `Kids'`; `ll` where titles write `Laceless`; `in` where titles write `Indoor`; `tf` where titles write `Turf`. The abbreviations are lossy in both directions, since `in` is also a substring of ordinary words and `mid` appears at some surfaces and not others (`strategy/sprint-backlog.md` B-STRAT-01, the cut-token filter sweep 2026-08-14).

**A related trap in the same family, confirmed 2026-08-25:** Shopify option NAMES are not attribute sources either. `adidas-kids-f50-hyperfast-club-turf-soccer-shoes-chaos-vs-control` labels its size option `Kid's Shoe Size` while its Immortal DNA pack sibling labels the identical 10.5K to 13.5K range `Youth Shoe Size`. Read as an age-band signal, that difference invents a config split that does not exist and suppresses a real pack collision. Compare the variant VALUES, not the option label.

**Practical procedure.** At Phase 0, record the live title verbatim per SKU and derive tier, cut, surface, age band, closure and gender from it. Where the handle and the title disagree, the title wins and the disagreement is logged on the brief. Copy wording follows the live title (`SEO_BATCH_PROCESS.md` section 2); handles in the Step 2 handoff still come from the brief verbatim.

## Pack succession and PDP keyword ownership (v2, added 2026-08-04)

Applies to footwear PDPs for models that ship in multiple packs. Nike, adidas, Puma, and others release several packs per year of the same model + tier + cut + surface, differing only on pack name and colorway, and ProSoccer carries multiple concurrently. Approved by Mike 2026-08-04. This makes the incumbent-keeps-it precedent (JR5386 / KJ3432) a written rule, and retires the Batch 12 shadow-suffix omission rationale (point 7). Mike rejected the broader "every pack PDP qualifies from launch" form because it moved working pages (for example JR5386 holding "adidas f50 elite" at 6,600) onto unoptimized collections and assigned near-every footwear PDP a zero-volume primary.

**1. Trigger.** Pack-qualification is required ONLY when a concurrent LIVE pack sibling exists for the same **age band + model + tier + cut + surface**. "Live" means a published, purchasable product page (stock on the product or any variant). Concurrency is checked against the live sitemap, not the registry alone: incumbents are often unoptimized and therefore absent from `products-master.csv`, so a registry-only cannibalization check cannot see them (point 8).

**AGE BAND is part of the configuration tuple (added 2026-08-25, Mike: rule text corrected to match practice).** Adult, Women's, Junior and Kids pages at the same model + tier + cut + surface are DIFFERENT configurations, not pack siblings, and they take separate primaries. This was always how the registry assigned terms (`adidas f50 club` on the adult page and `adidas junior f50 club` on the junior page are separate held primaries, not a collision), but the tuple as originally written named only model + tier + cut + surface, so the practice was correct and the rule was silent. Batch 15.1 is the batch that made the omission bite: ten of ten SKUs are junior, kids or women's, so age band is the primary distinguisher across the entire batch and the tuple that governs it did not mention the field.

Read age band off the LIVE TITLE, never the handle, and confirm it against the variant size VALUES rather than the Shopify option label (see 'The live title governs' above). Worked case: `adidas-kids-f50-hyperfast-club-turf-...-chaos-vs-control` and `...-immortal-dna` label their size option `Kid's Shoe Size` and `Youth Shoe Size` respectively while carrying the identical 10.5K to 13.5K range. They are the same age band and therefore genuine pack siblings; the labels say otherwise and the labels are wrong. Conversely `adidas-kids-f50-hyperfast-club-fg-mg-...` (Kids, 8K to 13.5K, Velcro) and `adidas-jr-f50-hyperfast-club-fg-mg-...` (Junior, 1 to 6, laced) share a pack, tier and surface but are different age bands AND different closures, so neither qualifies against the other.

**2. Assignment.**
- No live pack sibling: the PDP keeps the unqualified age band + model + tier + cut + surface term. It resolves to one product today, which is the resolution test. This is the common case, and it is why this rule does NOT saddle every footwear PDP with a zero-volume primary.
- A live pack sibling exists: the incumbent keeps the unqualified term, and every newer pack takes a pack-qualified primary (age band + model + tier + cut + surface + pack). This is the JR5386 / KJ3432 shape, generalized.
- The pack qualifier carries the pack name always, and carries the season code ONLY when a live same-pack sibling from another season exists for the same age band + model + tier + cut + surface ("shadow fa26" versus "shadow fa25"), so two same-pack pages never collide on the same qualified term. Recurrence is judged per configuration, not per pack name in general: Breakout recurs as a pack, but not for the Tiempo Maestro Academy Turf, so IQ2388 takes `nike tiempo maestro academy turf breakout` with no season code. Confirmed necessary on the live store 2026-08-04 (a Shadow FA25 and a Shadow FA26 of the same shoe both live) and re-confirmed 2026-08-13 for HQ2275. Tightened 2026-08-13 from "when a pack name recurs across seasons", which read as pack-wide rather than configuration-scoped. Full rule, check method, and three worked examples: `context/page-type-playbooks/product-page-playbook.md` 'Pack season-code qualifier (added 2026-08-13)'.

**3. Incumbent (v3, redefined 2026-08-18 by Mike: measured demand, not season order).**
...
**SCOPE LIMIT FOUND 2026-08-28, and it does not overturn the ruling but it bounds it.** **100% of GSC `MERCHANT_LISTINGS` impressions are reported against `?variant=` URLs. Zero are canonical.** Measured across 12,543 merchant-listing page rows: every one carries a query string. **So canonical-only analysis is blind to the entire free-product-listing surface**, which over 90 days ran 678,578 impressions at 3.529% CTR and average position 3.49, four times the CTR of ordinary organic. The B-DETECT-01 detection-gap analysis, its 1,280 untracked products and every CTR figure in it, are therefore ORGANIC AND PRODUCT-SNIPPET ONLY. Not wrong, but narrower than the framing implied.<br><br>**The ruling still stands for its stated purpose**, which is choosing which page to write copy for: a variant URL is still the same document and optimizing the canonical still does not change what those visitors see. **What changes is that 'canonical impressions' must never be described as 'the page's impressions' or 'what the page earns'.** They are what the page earns IN ORGANIC. Any question about merchant listings, free listings or Shopping surfaces must deliberately include query-string rows and say so. When a measurement excludes a surface, the exclusion belongs in the reported figure, not only in the method. Origin: B-TECH-03, `deliverables/technical-fixes/2026-08-28_merchant-listings-diagnostic.md`.
...
**CHURN GUARD (non-negotiable).** Incumbency does not move on a quarterly reshuffle. The stability the season-earliest rule provided is what this clause replaces, and it is the reason the redefinition is safe.
- Once assigned, incumbency moves ONLY on sustained divergence across **two consecutive measurement periods**. One period of a challenger leading is noise; two is a signal.
- Incumbency **never** moves for a shipped page without an explicit decision from Mike, recorded per page. Point 9 (no retroactive churn) governs, unchanged.
- A pack rotation, a seasonal spike, or a single window's reordering is not a reassignment trigger. Demand at these configs is measurably spiky: the Tiempo Ligera Pro FG config had its lead change hands with a +748% swing in one 29-day window while the config total fell 32%.
...
- exactly 1 remains: the unqualified term succeeds to that surviving PDP, which becomes the new incumbent (it resolves to one product again);
- **2 or more remain: the term succeeds to the highest-earning live sibling by trailing-90-day GSC impressions, which becomes the new incumbent.** Where the survivors are all below the measurement threshold (see point 3), the term moves to the most specific existing model collection instead, which is the pre-v3 behaviour and stays correct when there is no measured signal to choose on.
- 0 remain (model discontinued): the term moves to the model collection, or its 301 target.

The 2-or-more branch changed with the v3 incumbent definition. The old rule sent the term to a collection because "picking one sibling would recreate the ad-hoc problem." That objection was about picking ARBITRARILY. Measured demand is not arbitrary, so where a clear earner exists the term should go to it rather than to a collection that may not be optimized (the pattern behind B-CAT-01: handing a term to an unprepared collection is a net subtraction on the day it ships). Where there is no clear earner the original objection stands untouched and the term still goes to the collection. The churn guard in point 3 applies to any such reassignment.
No trade-off objection applies here: at archival the incumbent page is already gone, so nothing ranking is being sacrificed. A periodic check must flag registry rows whose product is archived or whose URL 404s or 301s while `normalized_primary` is unqualified, or the term silently orphans. This monitoring hook is not built yet (Reporting / registry item).

**5. Sub-floor lock (a specific case of the general lock).** When a live sibling forces qualification, the pack-qualified sub-floor primary is MANDATORY and correct. Do not walk the fallback hierarchy down to a floor-clearing term, and do not route back to KIRA: a less-specific term collides with the incumbent or the sibling. This is the general sub-floor-lock doctrine (`SEO_BATCH_PROCESS.md` section 3, "when no floor-clearing term is hierarchy-valid, the page takes the exact qualified term and is flagged sub-floor"; operating rule since Batch 9), applied to packs. It is NOT an exception to a route-back posture: the general rule is also the lock (see 'Hierarchy bound and sub-floor lock' in the Volume-Weighted Primary Keyword Selection Discipline above, and `.claude/agents/master-strategist/agent.md` Section 9).

**6. Volume honesty.** This rule assigns a zero-volume qualified primary only in the genuine-collision case, where the page cannot own the shared term anyway. An unmeasurable term is absent, not winnable, so it does not inherit the "a term you can win beats one you cannot" justification; it is accepted only under forced qualification. Model-term demand (which for adidas reaches down to model + tier + surface, and for Nike lives at the model head) is captured by the model collection, which survives pack EOL via the Shopify 301, not by the pack PDP. The pack PDP is a conversion and pack-navigational surface. DFS confirmation 2026-08-04: pack-qualified terms are near-universally zero (only the current flagship pack, "nike phantom 6 shadow pack", scraped 50/mo, launch-inflated).

**7. Retires the Batch 12 shadow-suffix omission rationale.** Batch 12 dropped the pack on six Nike Academy and Club SKUs on the reasoning that cut and surface uniquely identify the SKU. The live sitemap contradicts that premise: each of the six has a concurrent live senior pack sibling (Breakout SU26, Attack SP26, Scary Good FA25, Break 'Em, or a prior Shadow FA25). The pack is the actual distinguisher. Those six shipped pages are grandfathered under point 9, not force-corrected; the qualify-or-grandfather call is logged as `strategy/sprint-backlog.md` B-PACK-01 for Mike.

**8. Live-sitemap collision check.** The gate cannibalization check and the registry pre-dispatch pass are exact-match against `products-master.csv`, which holds optimized pages only. For footwear packs, add a live-sitemap read (firecrawl_map over prosoccer.com, or the product sitemap) for the age band + model + tier + cut + surface stem before assigning a primary, so unregistered concurrent incumbents are seen. Without it, the registry reports clean on a collision it cannot see. **That live-sitemap read must itself assert snapshot freshness before it is trusted** (codification checklist item 12): a stale cache makes this exact check report clean on a collision it also cannot see, which is the Batch 15.1 Sparkfusion case.

**9. No retroactive churn.** The rule applies to new pages. The incumbent never moves. Shipped pages are not retargeted on account of this rule alone; existing exceptions are logged (sprint-backlog), not force-corrected. Mike decides any shipped-page retarget per page.

Cross-references: `SEO_BATCH_PROCESS.md` section 3 (Keyword hierarchy, PDP clause updated 2026-08-04); 'Volume-Weighted Primary Keyword Selection Discipline' and 'Mechanism C: pack/colorway/release-specific secondary keyword discipline' above (a pack term is a secondary by default and a primary only under this rule); `shared-intelligence/seo-findings.md` 2026-08-04 entry (live-concurrent-packs finding and DFS volume shape); `strategy/sprint-backlog.md` B-PACK-01 / B-MERCH-01.
...
5. **Reconcile cross-file contradictions, not just intra-file drift (added 2026-08-04, recurring pattern).** A rule can be written correctly in one canonical file and contradicted in another, with agents following whichever they happen to read. This is a recurring CLASS, not a one-off: the FAQ-heading rule, the Meta Title rule, and the PDP store-CTA rule each shipped a contradiction between a playbook and an agent definition or `03-brand-voice.md`. Batch 12: `product-page-playbook.md` forbade the Pasadena fitting-room CTA on PDPs while `.claude/agents/on-page-seo/agent.md` and `03-brand-voice.md` encouraged it, so two SCRIBEs in one batch treated it oppositely. When you write or change a rule, grep the OTHER canonical files (the sibling playbooks, the agent definitions, `03-brand-voice.md`) for the same subject and reconcile them in the SAME change. Actively audit for this class; do not wait to catch it in shipped output. **Recurring locus (added 2026-08-04): the agent definition files (`.claude/agents/*/agent.md`) are the repeat offenders**, because they were written before several doctrines settled and are not re-read end-to-end each batch the way the playbooks are. The sub-floor-lock contradiction (fifth instance of this class) lived in `master-strategist/agent.md` Section 9's "route back rather than lock a sub-floor primary" phrasing, written 2026-06-09, months before the lock doctrine settled at Batch 9. When you codify or change a keyword or content doctrine, grep the agent files specifically, not just the playbooks.

6. **An agent's self-report about its own output is not evidence. Only the deterministic check is (added 2026-08-13).** Any figure or pass claim that reaches a report must be traceable to a check that RAN, not to the agent that produced the work. This is `SEO_BATCH_PROCESS.md` section 5 rule 1 ("A report is not verification") extended to self-measurement specifically, and it is a recurring CLASS with three instances: Batch 9, where "all 10 in band" was reported while five briefs were over; Batch 12, where SCRIBEs self-reported clean on three defects the gate then caught; and Batch 14, where SCRIBE word counts disagreed with `batch_gate.body_word_count` by up to 23 words in both directions. The failure is structural, not careless: an agent measuring its own output uses its own method, and the method is exactly what the deterministic check exists to standardize. Practical rule for ORIN: when a subagent reports a number, re-derive it from the gate or the script before it enters a report to Mike, a commit message, or a registry row. A subagent's self-report is a progress signal, never a measurement.

7. **An exemplar must be traceable to authored, gate-passed copy. Provenance, not just compliance (added 2026-08-14).** Before a string becomes an exemplar in any canonical file, name its source: which brief authored it, did it pass the gate, and is it live. A string that satisfies every written rule can still be the wrong thing to teach, because compliance says the string is well-formed and says nothing about where it came from. **Two failure shapes, both found in one section on 2026-08-14** (`product-page-playbook.md` 'Meta Title and Meta Description compliance'):
   - **Lifted from a live page.** The cleat Meta Description exemplar was the PRE-EXISTING meta on a Phantom 6 High Elite page. Both of that SKU's briefs record the string as the page's existing copy, and the live audit then showed the page was never optimized at all. So the canonical example of our house style was white-label copy we did not write, and every SCRIBE had been calibrating against it.
   - **Invented in the playbook.** The jersey Meta Description exemplar appears in no brief anywhere in the repo. It was written straight into the canonical file, never produced by SCRIBE, never gated. It also spelled `CLIMACOOL` in all caps, which the brand-technology title-case rule (added 2026-08-13, in this same repo) forbids, so the example taught a violation of a rule stated elsewhere.
   
   This is the FOURTH instance of the exemplar class, after the 20 Meta Title brand-suffix violations, the capitalized `Adidas` inside fenced blocks, and the UK `boot` in the silo files. The first three were compliance drift between a rule and its example. This one is different and worse: the examples were compliant and still wrong, because nothing ever asked where they came from. **Practical rule: when you write or change an exemplar, record its SKU, its brief path, and its live-verification date beside it, exactly as that playbook section now does. If you cannot name the source, it is not an exemplar.** A worked example that models a real shipped page is evidence; one you compose to illustrate a rule is just the rule again, with the added risk that it drifts from every other rule in the file.

8. **An exemplar must be checked against EVERY rule in the repo, not just the rule it illustrates (added 2026-08-14). This is the mechanism behind all four instances.** An example is written to demonstrate one rule, so it gets reviewed against that rule and no other, and it quietly violates the rest. Four found in one sweep, each illustrating a rule it satisfied while breaking rules stated elsewhere in the same repo:
   - A Meta Description exemplar, compliant on length and opener, spelled `CLIMACOOL` all-caps against the brand-technology title-case rule.
   - A Meta Title exemplar for a collection, `Adidas Predator Cleats | Pro Edition, Elite & 24 Models`, demonstrating title construction while being a comma-stacked, ampersand-terminated keyword list, which is anti-patterns 1 and 2 of this file's own anti-stuffing discipline.
   - A homepage Meta Title exemplar typing the store name the theme already appends, which the meta rule forbids.
   - A jersey worked example teaching structure while stating that customization happens "through the storefront" and adds "five to seven business days", both contradicting `context/shipping-customization-facts.md` and both the exact defect `batch_gate.check_customization_claims` exists to catch.
...
11. **A new artifact is not verified until it is confirmed present in a FRESH CLONE, not merely present locally (added 2026-08-19, Mike).**
    ...
    **FIFTH AND SIXTH INSTANCES, both in one audit, added 2026-08-27 (Mike). THE INSTRUMENT REPORTED AND THE REPORT WAS THE ARTIFACT.** Same class as the truncated-url probe and the single-endpoint DFS pull. Both were self-caught during the Batch 16 colorway audit, before the numbers reached anyone, and both are recorded because a self-caught instrument error is the cheapest possible example of one.

    - **A colour-matching pattern with no word boundaries.** `red` matched inside **P-red-ator**, `engineered` and `hundred`, so every Predator brief in the corpus registered a colour claim it never made. The first reported figure was **78 briefs**; the true figure is **67**. Eleven false positives, and the four Batch 16 Predators were among them, which is what made it noticeable: a batch whose briefs had just been written and read did not plausibly contain colour claims. The fix is one token, `\\b(?:...)\\b`, and the lesson is that a regex over prose is an instrument and needs a control like any other.
    - **A context window that silently dropped short lines.** The verification grep asked for 70 characters on both sides of the match, so any match on a line shorter than that returned NOTHING rather than returning the match with less context. Two briefs read as having no colour sentence at all when both in fact had one. Had that gone unchallenged it would have produced two false NEGATIVES, which is the worse direction: a false positive gets investigated, a false negative closes the file.

    **SEVENTH INSTANCE, and this one is in a TOOL rather than in something we wrote, added 2026-08-27 (Mike): the GSC MCP tool's `regexFilter` parameter is SILENTLY IGNORED.**

    `mcp__gsc-server__search_analytics` accepts a `regexFilter` argument. Passing it alongside `filterOperator: "includingRegex"` returns **HTTP 200 and a full, plausible, completely UNFILTERED result set**. A Batch 16 close query passed a tight alternation of nineteen club head terms and got back the site's global top rows: `prosoccer`, `pro soccer`, `pro soccer pasadena`, `soccer jerseys`. Nothing errored, nothing warned, and every row was real data.

    **That is the worst possible failure shape and it is exactly the truncated-url probe again.** The response is well-formed, the numbers are genuine, and only the SCOPE is wrong. Had the query been for something whose right answer was not obvious, the unfiltered set would have been analysed as though it were the filtered one. It was caught only because `prosoccer` is self-evidently not a club head term.

    **THE WORKING FORM: put the regex in `queryFilter` and set `filterOperator: "includingRegex"`.** `queryFilter` is honoured. `regexFilter` is not. Anchor the pattern (`^(a|b|c)$`) for exact-term work, or the alternation matches every long-tail containing a term and inflates the row count.

    **The general rule this instance argues for: when a filter parameter is available, verify it FILTERED before trusting the result.** The cheap check is to confirm the response contains only rows that satisfy the filter, or to run the same query with a filter that should return nothing and confirm it does. A filter that silently no-ops is indistinguishable from a filter that matched everything, and both return confident numbers.

    The pairing is the point. The same audit produced an over-count from a pattern that matched too much and a near under-count from a probe that displayed too little, and neither instrument announced anything was wrong. **Before trusting a count you derived, run it against a case whose answer you already know.** The Batch 16 briefs were that control here, available for free, and consulting them earlier would have caught the first error immediately.

    **EIGHTH INSTANCE, added 2026-08-27 (Mike), and it is the seventh one layer up: A CAPPED RESULT SET IS INDISTINGUISHABLE FROM A COMPLETE ONE, AND THE SORT ORDER SILENTLY DECIDES WHICH ROWS SURVIVE.**

    The B-DETECT-01 pull asked GSC for every canonical URL over 1,000 impressions not in the registry. It passed `rowLimit=1500` and got back **exactly 1,500 rows**. Nothing errored, nothing warned, every row was real, and the derived answer, 599 untracked products, was filed as a finding. The re-run paginated to exhaustion and returned **31,643 rows and 1,280 untracked products**. The first number was not 47% of the truth by accident of scale; it was a different question answered confidently.

    **The mechanism is the dangerous part, and it is not "the number was low".** `searchanalytics.query` returns rows **sorted by clicks descending and exposes no sort parameter**, so a row cap does not trim the least important rows, it trims the rows with the fewest CLICKS. The last surviving row carried 12 clicks, so every page under 12 clicks was deleted regardless of its impressions. **The query was hunting pages with high impressions and near-zero clicks, and the cap deleted exactly that population.** The instrument was selecting against the finding. `puma-23-24-chivas-mens-away-promo-jersey`, the page the entire B-EQUITY-01 thread rests on, has 13 clicks and survived by ONE. 682 products were newly surfaced, and 658 of them sat under that 12-click line.

    **Why it is the same class as the silently ignored `regexFilter`, not a lesser cousin.** Both return HTTP 200, well-formed rows, and genuine numbers. In the `regexFilter` case the SCOPE was wrong; here the COMPLETENESS is wrong. Neither announces itself, and both are caught only by asking a question the response cannot answer about itself. The `regexFilter` no-op was caught because `prosoccer` is self-evidently not a club head term. This one had no such tell, because a truncated list of real untracked pages looks exactly like a complete list of real untracked pages. It survived a full write-up, a commit and a push.

    **THE RULE: when a result set can be capped, prove it was not.** Compare the returned row count against the limit you requested. **If they are equal, you have a page of results, not an answer.** Paginate until a call returns fewer rows than the limit. State the total and the number of calls in whatever you write, so the next reader can tell exhaustion from truncation without re-running anything.

    **AND THE SECOND RULE, which is the one that generalizes past pagination: know what your instrument's ORDERING is, and ask whether it correlates with what you are looking for.** A cap is harmless when the sort is aligned with the question and lethal when it is anti-correlated. Sorting by clicks while hunting for low-click pages is the worst available case, and nothing in the API surface says so. Before trusting any truncated or ranked result, ask: **if the thing I am looking for existed, would this ordering have shown it to me?** Here the honest answer was no, and it was answerable in advance.

    **THE STOPPING RULE, stated generally because it is not about this pull (elevated 2026-08-27, Mike).**

    **When the SORT key of a result set differs from the FILTER key of your question, position in that result set carries no information about your filter, and exhaustion is the only defensible stopping condition.** Not a lower bound, not a sampled cut, not a threshold on the filter key. Exhaustion.

    The reasoning is short. A stopping threshold is a bet that everything past this point is smaller than what I care about. That bet is only payable when the ordering guarantees it, which means when you are stopping on the same key the data is sorted by. Stop at 1,000 impressions in an impressions-sorted set and you have lost nothing. Stop at 1,000 impressions in a CLICKS-sorted set and you have lost an unknown quantity, because the next row could carry 700,000 impressions and two clicks. **The threshold looks like rigour and is arithmetically meaningless.**

    B-DETECT-01 is the worked example in both directions. The click-sorted cap deleted 682 untracked products, 658 of them under a 12-click line, in a query whose entire purpose was finding high-impression low-click pages. And when an impressions threshold was proposed for the re-run it was declined on this reasoning rather than applied, which is the rule working forward instead of being learned again.

    **Practical form: before accepting any stopping rule, name the sort key and the filter key out loud. If they are not the same key, the rule is unsound and the honest answer is to paginate to exhaustion and say how many calls it took.** If exhaustion is genuinely unaffordable, that is a budget decision to surface, not a threshold to invent: say what was not read, rather than implying it was nothing.

    **NINTH INSTANCE, added 2026-08-27, self-caught within minutes and recorded because the detector was one WE built for this exact defect: A DETECTOR THAT FALSE-POSITIVES IS NOT HARMLESS WHEN SOMETHING AUTOMATED ACTS ON IT.**

    Commit d2ab207 fixed two backlog rows that had been glued onto the end of the previous line, and added a check that re-derives from disk with a regex for rows not at line start: `(?<!\n)\| B-[A-Z]+-\d`. On 2026-08-27 that regex fired on a correctly formed file, because a B- id is a legitimate **column value**: the new B-MERCH-05 row cites `| B-DETECT-01 stock resolution 2026-08-27` in its source column, and mid-row is precisely where the pattern says a glued row lives. The row was fine. The detector said otherwise, and the repair script acting on it inserted a newline **into the middle of a valid row, splitting it in half**. A file that was correct before the check ran was broken by the check's remedy.

    The tell was that the repair reported fixing `| B-DETECT-0`, an id that is not a row in that position, and the post-repair verification then showed 13 malformed rows where there had been 11. Restoring from HEAD and re-applying cost one cycle. **The insert itself had been correct all along**, which is the part worth keeping: the original edit passed, the check failed it, and the automated correction did the damage.

    **The rule: a check that can fire on a correct file must never be wired to an automatic repair.** Detection and remediation have different error costs. A false positive in a detector costs a human glance; the same false positive in an auto-fixer costs data. **Where remediation is automated, the detector must be decidable at the granularity of the thing it edits.** Row-start is a LINE-level fact, so it must be tested line by line (`line.startswith('| B-')`), never by scanning a whole document for a pattern that a legitimate cell value can satisfy. This is the same family as item 10's test-asserts-the-bug and item 11's colour regex without word boundaries: an instrument confidently reporting on the wrong subject. The difference is that this one had a write primitive attached.

    **AND THE DISTINCTION MIKE DREW ON IT, 2026-08-27, which is the reason this instance is worth more than its size: THIS IS A DIFFERENT FAILURE FROM A CHECK THAT MISSES SOMETHING, AND IT IS WORSE.** Every other instance in this item is a check that failed to catch a real defect, or a tool that returned a wrong answer to a question. Those cost you a defect that ships. **This one broke correct work.** The file was right, the check was wrong, and the system acted on the check. A missed defect leaves you where you started; an automated repair driven by a false positive leaves you worse than if the check had never existed. **When weighing whether to automate a remediation, the question is not how often the check is right. It is what the check does to a correct file when it is wrong.**

    **TENTH INSTANCE, added 2026-08-27 (Mike): AN LLM READING STRUCTURED DATA STILL PRODUCES A REPORT, NOT A MEASUREMENT, AND THIS ONE WAS NEARLY RIGHT IN A WAY THAT WOULD HAVE SURVIVED A CASUAL CHECK.**

    Stock for the B-DETECT-01 shortlist was read through Firecrawl's `json` format against each product's `.js` endpoint. That format is an LLM extracting from a document, and the document here was already machine-readable JSON, which is precisely why the result felt safe. **On 4 of 25 products it miscounted the `variants` array, reporting one more option than the product has.** The shipped figure was 133 total variants; the true figure is **129**. The wrong number reached a backlog item, a tracking deliverable, a commit message and a brief prepared for a third party.

    **What makes it a good example is that it was nearly right.** Not a hallucinated product, not a wild count: an off-by-one on four rows out of twenty-five, in a plausible direction, on a figure nobody would independently recompute. A spot check of one or two rows would very likely have hit one of the 21 correct ones and confirmed the whole set. It was caught only because an INDEPENDENT source, a bulk `products.json` pull, was later obtained for a different reason and disagreed, and the disagreement was then settled by a direct `.js` fetch counted programmatically.

    **The sold-out verdict never moved.** Both sources put all 25 pages at zero available variants, agreeing 25 of 25 on the binary. Only the denominator was wrong. That is worth stating because it shows the failure is not 'the LLM was unreliable' in general; it was unreliable at COUNTING while being reliable at reading.

        **ELEVENTH INSTANCE, added 2026-08-28 (Mike), and it is the third this week of one shape: A PLAUSIBLE NUMBER DERIVED FROM THE WRONG BASE, SURVIVING BECAUSE NOBODY RECOMPUTES A FIGURE THAT LOOKS REASONABLE.**

    I reported that merchant listings were '8.2% of impressions'. The denominator I used, 8,247,869, was the SUM OF THE SEARCH-APPEARANCE ROWS. The actual site total is **19,842,113**, because **58.4% of impressions carry no recognised appearance type at all** and therefore appear in no appearance row. The true figure is **3.4%**. Every share quoted from that base was inflated about 2.4x, and the wrong number reached a backlog item, a commit message and a brief written for Mike to act on.

    **Nothing about 8.2% looked wrong.** It was the right order of magnitude, it supported a conclusion that was independently true (merchant listings convert far better than organic), and it was arithmetically correct given its input. **That is the whole failure mode: a figure that is wrong only in its denominator is indistinguishable from a right one by inspection.** It was caught only because a later question required the site total for a different reason, and the two numbers did not match.

    **This is the third instance in one week of the same shape.** The colour-matching regex without word boundaries counted the wrong population. The LLM variant count read the right document and miscounted its array. This one divided by the wrong total. **In all three the instrument produced a plausible number and no error surfaced anywhere.**

    **The rule: when you report a proportion, state the denominator beside it and derive that denominator independently of the numerator.** 'X of Y' is auditable; 'X%' is not. Where the numerator comes from a filtered, grouped or dimensioned query, do NOT obtain the denominator by summing the same query's rows: ask the source for the total separately and with no dimensions. Summed rows are a subset whenever the grouping is optional, and in GSC's `searchAppearance` the grouping is optional for more than half of all impressions.

    **AND THE COMPANION RULE, which is Mike's wording and generalises past denominators: WHEN A MEASUREMENT EXCLUDES A SURFACE, THE EXCLUSION BELONGS IN THE REPORTED FIGURE, NOT ONLY IN THE METHOD. 'CANONICAL IMPRESSIONS' IS NOT 'THE PAGE'S IMPRESSIONS'.**

    The canonical-only ruling is correct for its purpose and was correctly applied. But **100% of merchant-listing impressions are reported on `?variant=` URLs**, so canonical-only analysis is structurally blind to free listings, and B-DETECT-01 described its results as 'untracked pages earning impressions' rather than 'earning ORGANIC impressions'. The method was documented and the figure was not, so every downstream reader inherited a scope they could not see. The Guatemala PDP reads as a total failure at 21 canonical clicks while taking 35 clicks off variant URLs, 26 of them from merchant listings: **the exclusion did not just narrow the number, it inverted the conclusion for that page.**

    Practical form: **name the surface in the figure itself.** Not 'impressions' but 'organic impressions'; not 'the page earns X' but 'the page earns X in organic, excluding free listings'. If a label would need a footnote to be honest, put the footnote in the label. Item titles count as labels: B-DETECT-01 was retitled on 2026-08-28 for exactly this reason.

**The rule: when a deterministic parse is available, never let a model do the arithmetic.** `json.load` and `len()` cannot miscount an array. Reach for LLM extraction when the source is unstructured prose and there is no parser; the moment the source is JSON, XML, CSV or any parseable format, parse it. This is checklist item 6 (an agent's self-report is not evidence, only the deterministic check is) extended to the case that feels exempt, because the model is reading structured data and therefore looks like it is parsing. It is not parsing. It is reading, and then reporting what it read.
    ...
    **A GUARD THAT FORCES AN INPUT IS NOT A GUARD THAT VALIDATES IT (added 2026-08-25, Mike). The `--date` hardening did not prevent the regression it was built for.**

    Requiring `--date` stops the tool inventing a date. It does not stop the CALLER supplying a wrong one, and the wrong one is the common case, because a regeneration is almost always triggered by adding one new row to a file full of old ones. On 2026-08-25 a run passing the honest, current date (`--date 2026-08-25`, to record six new Sparkfusion cedes) rewrote the recording date of **all 24 pre-existing derived rows** from their real 2026-08-03. Same regression, same invisibility: the rows' content was byte-identical and only the dates moved. It was caught by diffing the regenerated file against the committed one, which means the tool still depends on a human remembering to diff, and that is not a guard.

    The distinction to carry forward: **forcing an input converts a silent default into a visible argument; it does not make the argument correct.** A guard is only complete when something can tell right from wrong, not merely present from absent. Ask of every required-argument guard: what would happen if the caller passed a plausible but wrong value? If the answer is "the same silent corruption the guard was added to prevent," the guard is a prompt, not a check.
...
12. **A check whose correctness depends on a cached snapshot must ASSERT that snapshot's freshness before running, and FAIL if it is stale. Silence about age is not evidence of currency (added 2026-08-25, Mike).**
    ...
    Item 9 closed the case where a check cannot run. This is the different and quieter failure: a check that runs perfectly, against stale input, and returns a confident wrong answer. Nothing skips, nothing prints, the gate exits 0, and the output is indistinguishable from a correct one. The human backstop cannot catch it because there is no anomaly to read.

    **The case, Batch 15.1 pre-dispatch 2026-08-25.** The live-sitemap pack collision check (pack succession point 8) reads `scripts/build/sitemap-cache/`. That cache was 12 days old, against the generator's own documented weekly-Monday refresh discipline. In those 12 days the store gained 127 products, two of which were `adidas-f50-sparkfusion-elite-fg-ag-soccer-cleats-trinity-rodman` and `...-league-fg-ag-...-trinity-rodman`. Those two ARE the pack siblings of the batch's two Sparkfusion SKUs. Against the stale cache the check reports "no live pack sibling" for both, both take unqualified primaries, and both are wrong: measured GSC puts the actual incumbents at 1,126 and 4,351 impressions against the batch pages' 27 and 241. **The check would have run, passed, and been wrong, with nothing anywhere to signal it.** It was caught only because the freshness was questioned before the check was trusted, which is exactly the discipline this item now makes mandatory rather than optional.

    **The rule.** Any check, script, or pre-dispatch pass that reads a cached snapshot must (a) know the snapshot's as-of timestamp, (b) compare it against a stated maximum age for that decision class, and (c) FAIL, not warn, when the snapshot exceeds it. A snapshot with no recoverable timestamp is stale by definition and fails on that ground alone. Refusing to run is correct; running against stale input and reporting a verdict is not.

    Note the asymmetry that makes this worth a rule: staleness is invisible in the OUTPUT and cheap to check at the INPUT. Every instance of this class will look like a clean pass.
...
- 'Pack succession and PDP keyword ownership (v2, added 2026-08-04)' above is the canonical rule for footwear pack keyword ownership; `SEO_BATCH_PROCESS.md` section 3 (Keyword hierarchy) and `.claude/agents/master-strategist/agent.md` Section 9 (volume-floor exception) carry the reconciled cross-references.
