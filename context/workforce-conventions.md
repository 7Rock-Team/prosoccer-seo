...
**3. Incumbent.** The incumbent is the earliest-shipped live pack for that model + tier + cut + surface, by season code (FA25 < HO25 < SP26 < SU26 < FA26, earlier year first). Ties break by launch date, then by the higher-ranking live page. The incumbent does not move when later packs ship.

**4. Succession when the incumbent is archived.** Live-as-long-as-stock means an incumbent eventually sells out and is archived while holding the unqualified term. Reassign by the count of live pack siblings remaining at that moment:
...
- 2 or more remain: the term moves to the most specific existing model collection (picking one sibling would recreate the ad-hoc problem, and the term genuinely describes several products);
