...
- **GSC MCP, `mcp__gsc-server__*`** (Category A). Google Search Console query analytics for the owned property, registered as the domain property `sc-domain:prosoccer.com` (this exact `siteUrl` is required on every call). Package `mcp-server-gsc` (ahonn) over stdio; auth is a Google service account (`prosoccer-gsc-reader@prosoccer-seo-workforce`, `siteFullUser` read access) whose JSON key is referenced by `GOOGLE_APPLICATION_CREDENTIALS` in env and lives OUTSIDE the repo (`C:\Dev-Projects\.credentials\`), never committed. Tools: `list_properties`, `get_search_analytics`, `get_search_by_page_query`. Installed 2026-06-09, closing the pre-staged drift (the `gsc-server` prefix was already present in `settings.local.json` and in ORIN/SCRIBE/KIRA/VERITAS frontmatter before the server existed). **Verification (2026-06-09):** a direct GSC API smoke test returned the `sc-domain:prosoccer.com` property plus real query data (site-wide top queries and a page-filtered query on the Phantom 6 High Elite FG PDP), proving credential, property access, and data flow. The MCP-load and sub-agent-inheritance checks (Category A's distinguishing test) run after the next Claude Code restart loads the stdio server; treat the smoke test as credential-and-data proof, not yet MCP-wiring proof. **Distinction from Drive MCP:** Drive is Category B (parent-only OAuth); GSC is Category A, so sub-agents (SCRIBE, KIRA, VERITAS) inherit the env credential and call GSC directly, with no parent-fetches-and-passes workaround. **Workflow status:** callable now, but Phase 1 keyword-research integration is deferred to Commit 4 (volume-weighted primary keyword selection plus GSC integration); until then DataForSEO SERP remains the ranking method of record in the standard workflow.
...
All referenced MCP servers are now Operational above. GSC MCP, the last pending server, was installed 2026-06-09 (Category A, see Operational). Its Phase 1 keyword-research integration is deferred to Commit 4 (volume-weighted primary keyword selection plus GSC integration); until that lands, the standard workflow keeps these methods, now framed as current-until-Commit-4 rather than install-gap fallbacks:

- **Ranking context per page (primary keyword position lookup):** DataForSEO SERP API via `mcp__dfs-mcp__serp_organic_live_advanced` remains the ranking method of record for the Fresh Optimization workflow Step 4 (see 'Fresh Optimization workflow' above). Once Commit 4 wires GSC into Phase 1, ranking context shifts to GSC `get_search_analytics` per URL for the source-of-record advantage; DataForSEO SERP remains useful for competitor-context lookups but not for ProSoccer's own ranking baseline.
- **CTR ceiling diagnostics, query-by-page intersection, indexation state, Rich Results coverage:** GSC `get_search_analytics` / `get_search_by_page_query` are now callable for these; the coarse CSV exports under `data/gsc-exports/` (12-month `_top-pages.csv`, `_top-queries.csv`, `_search-appearance.csv`) remain a usable offline baseline. Mike refreshes the exports on cadence (target: monthly).
