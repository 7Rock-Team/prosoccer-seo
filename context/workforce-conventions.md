...
11. **A new artifact is not verified until it is confirmed present in a FRESH CLONE, not merely present locally (added 2026-08-19, Mike).**
    ...
    **A corollary for stopping rules.** The instruction to stop at an impressions threshold was unsound for the same reason and was not applied: because the sort is click-descending, position in the result set tells you nothing about impressions, so no impressions floor can license an early stop. **When the sort key and the filter key differ, exhaustion is the only defensible stopping condition.** Say so rather than inventing a threshold that reads as rigour.
