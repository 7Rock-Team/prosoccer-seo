...
**1. Trigger.** Pack-qualification is required ONLY when a concurrent LIVE pack sibling exists for the same model + tier + cut + surface. "Live" means a published, purchasable product page (stock on the product or any variant). Concurrency is checked against the live sitemap, not the registry alone: incumbents are often unoptimized and therefore absent from `products-master.csv`, so a registry-only cannibalization check cannot see them (point 8).

**2. Assignment.**
- No live pack sibling: the PDP keeps the unqualified model + tier + cut + surface term. It resolves to one product today, which is the resolution test. This is the common case, and it is why this rule does NOT saddle every footwear PDP with a zero-volume primary.
- A live pack sibling exists: the incumbent keeps the unqualified term, and every newer pack takes a pack-qualified primary (model + tier + cut + surface + pack). This is the JR5386 / KJ3432 shape, generalized.
- The pack qualifier carries the pack name always, and carries the season code ONLY when a live same-pack sibling from another season exists for the same model + tier + cut + surface ("shadow fa26" versus "shadow fa25"), so two same-pack pages never collide on the same qualified term. Recurrence is judged per configuration, not per pack name in general: Breakout recurs as a pack, but not for the Tiempo Maestro Academy Turf, so IQ2388 takes `nike tiempo maestro academy turf breakout` with no season code. Confirmed necessary on the live store 2026-08-04 (a Shadow FA25 and a Shadow FA26 of the same shoe both live) and re-confirmed 2026-08-13 for HQ2275. Tightened 2026-08-13 from "when a pack name recurs across seasons", which read as pack-wide rather than configuration-scoped. Full rule, check method, and three worked examples: `context/page-type-playbooks/product-page-playbook.md` 'Pack season-code qualifier (added 2026-08-13)'.

**3. Incumbent (v3, redefined 2026-08-18 by Mike: measured demand, not season order).**

The incumbent is the live page at that model + tier + cut + surface **earning the most GSC impressions over a trailing 90 days.**
