...
## Volume-weighted primary keyword selection + GSC (added 2026-06-09)

Collection pages select a head or category-level primary keyword (highest-volume relevant term), so the 100/mo DataForSEO floor rarely binds the way it does on an ultra-specific PDP long-tail. The discipline still applies: the primary must clear the floor, and GSC integration is especially valuable at collection scale because collections carry larger query footprints. KIRA's Phase 1 protocol reads GSC `search_analytics` for the collection URL (`pageFilter`, `dimensions: "query"`) to see which queries actually land on the collection, and `detect_quick_wins` (default positions 4 to 10, or `positionRangeMin: 11, positionRangeMax: 20` for page-2 momentum) to find striking-distance category terms. Composite scoring (DataForSEO volume + GSC impressions + GSC position + intent + ranking realism) picks the primary; supporting and long-tail terms deploy in the body per the keyword distribution discipline above. Where a collection's natural head term is ambiguous (several category framings compete), GSC's landing-query data breaks the tie. Full discipline and the 6-step KIRA protocol: `context/workforce-conventions.md` 'Volume-Weighted Primary Keyword Selection Discipline (added 2026-06-09)' and `.claude/agents/keyword-research/agent.md` Section 9. Forward-only from the next batch dispatch onward.
