...
- Variant availability, size runs, and stock state (added 2026-08-04, Batch 12 IU7734). Do not write the live size run ("available in L, XL, 2XL, and 3XL," "sizes S to XL"), "in stock," "only a few left," "back in stock," or any inventory count into body copy. Inventory and variant state change silently, so the copy goes stale the moment a size sells out; Shopify renders the live variant list and stock state on the page itself. This is the same decay logic as the pricing-in-body discipline (`context/workforce-conventions.md` 'Content evergreen-ness'). Fit and sizing GUIDANCE ("runs true to size," "size up a half for thick socks") is fine and encouraged; the specific available-size LIST is not.
...
Correct: Haloskin, Haloshell+, Primeknit, Strikeframe, Powerspine, FlyTouch, Sprintgrid.

**Nanostrike is the exception: it is TIER-CONDITIONAL, not one fixed string** (resolved 2026-08-18 against adidas.com). **Elite tier: `Nanostrike+`. League and below: `Nanostrike`.** Two adidas.com PDPs carry the identical sentence frame with one differing token: Predator Elite Fold-Over Tongue FG reads "Soccer cleats with Nanostrike+ tech for precision striking"; Predator League FG reads "Soccer cleats with Nanostrike technology for precision striking". Take the tier from the SKU, never from the live ProSoccer page: our own pages render it `NANOSTRIKE` in all caps at every tier, which carries the casing defect AND destroys the tier signal, so scrape-wins cannot resolve this one. Full rule: `context/workforce-conventions.md`, brand technology casing.

**SECOND CONFIRMED INSTANCE, and a NEW failure mode (Batch 16, 2026-08-26, Mike).** The rule above was written from a single case where the live page merely *destroyed* the tier signal by upcasing it. IH7154 (`adidas Predator League Firm Ground Soccer Cleats - Chaos Vs Control Pack (FA26)`) is worse: its live description asserts the **wrong tier token outright**, reading "Nanostrike+ technology combines innovative mesh…" on a **League** product. That is not a casing defect and not an erased distinction, it is an affirmatively incorrect claim, and a scrape-wins reading would have shipped it. Two same-batch pages confirm the tier rule is right and IH7154 is the outlier: IH7200 (League) reads "Nanostrike mesh" and IH7234 (Pro) reads "Nanostrike Pro mesh". **Write `Nanostrike` for IH7154.** The generalization: a live page can be wrong about a tier-conditional token in three distinct ways (upcased, absent, or wrong), so the "tier from the SKU, never from the live page" instruction is unconditional rather than a tie-breaker. Flagged to Jorge as a manufacturer-copy error worth correcting at source (see `strategy/sprint-backlog.md` B-MERCH-03).
