...
- **(A) Category A**: stdio transport plus env-variable credentials. Full sub-agent inheritance via Option B `mcpServers:` declarations. Explicitly classified in commit `0c6dbb3`: `dfs-mcp`, `firecrawl-mcp`, `tavily-mcp`. Verified working at sub-agent dispatch level during Phase C verification.
...
- **(A\*) Inferred Category A**: follows the Category A pattern (no `claude_ai_` prefix, stdio-style server) but was not explicitly annotated in commit `0c6dbb3`. Applies here to `gsc-server` and `plugin_playwright_playwright`. Inheritance behavior at the sub-agent dispatch level should be verified by test before being relied on operationally. Tracked in `work-log/follow-ups.md`.
