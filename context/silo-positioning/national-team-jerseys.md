...
- **Brand / IP:** adidas is the CURRENT supplier (January 2023 through the 2026 cycle, replacing Umbro). TIME-SENSITIVE. adidas holds a specific FIFA license for the 2026 World Cup, so FIFA / World Cup terminology IS permitted on this adidas 2026 World Cup PDP, in the past tense for that event (a specific event license, not a standing partnership; re-verify for any future tournament). The tournament-status evergreen discipline still applies: no "chasing the trophy," no "qualified for 2026," no live-bracket framing.
...
### Jordan (added 2026-07-11, from ORIN research for Batch 7 Kelme Away; first-ever first-nation entry, Mike-approved exception criterion 1)
- **Identity:** "Al-Nashama" (roughly "the chivalrous / brave ones"). Gloss it in copy. Gender-neutral, same discipline as Zmajevi and Les Léopards; do NOT coin a feminine variant.
- **Colors / motif:** flag-derived white / red / black / green. The kufiya (shemagh) diamond motif on the kit is a real, culturally-grounded design anchor; use it only as the SKU's Phase 0 scrape confirms (scrape-wins on the specific design).
- **Federation:** Jordan Football Association (JFA); AFC member.
- **Evergreen heritage anchors:** 2026 is Jordan's FIRST-ever appearance on the sport's biggest international stage, which is true and verifiable and therefore a usable anchor, but phrase it in CYCLE LANGUAGE ("their first appearance on the biggest international stage," "the debut cycle"), NEVER "first FIFA World Cup" or "first World Cup." Qualification sealed 3-0 over Oman is a verifiable evergreen detail. Secondary FIFA-free anchor: 2023 AFC Asian Cup RUNNERS-UP (reached the final, their best-ever continental result; an AFC competition, so it carries no FIFA / World Cup wordmark and is safe to name directly).
- **Brand / IP:** kit supplier is **Kelme**, TIME-SENSITIVE (recurs here as supplier, same as Bosnia; not its own silo, not a permanent identity). Kelme is non-adidas, so FIFA / "World Cup" / "World Cup 2026" terminology is FORBIDDEN (cycle language only, same rule as Bosnia and DR Congo). See `context/brand-ip-constraints.md`; `batch_gate.py` check #5 enforces it at the gate.
- **Home-vs-away scrape check (SKU 7651TX1919):** the white-label sheet lists this SKU as "Away," but ProSoccer's live Jordan collection anchors the Home on white with red kufiya-diamond sleeves and the Away on red/black. CONFIRM home-vs-away and the exact colorway from the Phase 0 scrape and write what the actual SKU shows; do NOT inherit the sheet's "Away" label if the scrape disagrees. Scrape wins.
