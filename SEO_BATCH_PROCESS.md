...
- `products-master.csv` `product_id` **holds the SKU, not the Shopify numeric ID**, on 178 of 187 rows. Three rows carry a genuine numeric Shopify ID (`18281`, `18278`, `18282`) and 6 are blank, so the column is mixed and cannot safely be read as either type. Nothing depends on it; the import file sources real numeric IDs from the Matrixify export instead. **Corrected 2026-09-02: this line previously said the column was populated on only 3 of 178 rows, which confused "numeric" with "populated" and understated it by a factor of sixty.**
